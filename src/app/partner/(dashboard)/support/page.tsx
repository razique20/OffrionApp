        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-end mb-4">
               <button 
                 ...
                 className="flex items-center justify-center gap-2 px-6 py-2.5 bg-premium-gradient text-white rounded-xl font-bold hover:scale-[1.02] shadow-xl shadow-primary/20 transition-all text-sm"
               >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-[calc(100vh-350px)]">
            {/* Ticket List */}
            <div className={cn(
              "lg:col-span-4 bg-card border border-border rounded-[32px] overflow-hidden flex flex-col shadow-sm",
            ...
            {/* Ticket Chat */}
            <div className={cn(
              "lg:col-span-8 bg-card border border-border rounded-[32px] overflow-hidden flex flex-col shadow-sm relative",
                 ...
                 <>
                   ...
                   <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-secondary/5">
                      <div className="flex flex-col gap-1 max-w-[85%] self-start">
                         <div className="p-4 rounded-2xl bg-secondary/40 border border-border text-xs leading-relaxed">
                      ...
                      {selectedTicket.responses.map((res: any, idx: number) => (
                        <div key={idx} className={cn(
                          ...
                          res.senderRole === 'partner' ? "self-end items-end" : "self-start"
                           ...
                           <div className={cn(
                             "p-4 rounded-2xl text-xs leading-relaxed border transition-all",
                   ...
                   <div className="p-4 bg-card border-t border-border">
                      <div className="relative">
                        <input 
                          ...
                          className="w-full bg-secondary/40 border border-border rounded-xl pl-4 pr-12 py-3 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium disabled:opacity-50"
                          value={replyMessage}
                        ...
                        <button 
                           ...
                           className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 bg-premium-gradient text-white rounded-lg shadow-lg shadow-primary/20"
                        >
              ...
              <form onSubmit={handleCreateTicket} className="p-8 space-y-4">
                 <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-2">
                       {['technical', 'billing', 'partnership', 'other'].map(cat => (
                         <button
                           ...
                           className={cn(
                             "py-2 px-3 rounded-xl border text-[10px] font-bold capitalize transition-all",
                             newTicket.category === cat ? "bg-primary/10 border-primary text-primary" : "bg-secondary/40 border-border text-muted-foreground"
                           )}
                         >
                           {cat}
                         </button>
                    ...
                    <input 
                      type="text" required
                      placeholder="Subject"
                      className="w-full bg-secondary/50 border border-border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium"
                    ...
                    <textarea 
                      ...
                      className="w-full bg-secondary/50 border border-border rounded-xl px-4 py-3 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium resize-none shadow-inner"
                      value={newTicket.message}
                      onChange={(e) => setNewTicket({...newTicket, message: e.target.value})}
                    />
                 </div>
                 <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => setShowNewTicket(false)} className="flex-1 py-3 bg-secondary rounded-xl font-bold text-xs">Cancel</button>
                    <button type="submit" disabled={submitting} className="flex-[2] py-3 bg-premium-gradient text-white rounded-xl font-bold text-xs shadow-lg shadow-primary/20">
