...
export function RedemptionPulse() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full bg-card border border-border rounded-[40px] overflow-hidden shadow-2xl">
        ...
        <div className="absolute inset-0 z-10 p-12 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-white tracking-tight">Ecosystem Pulse</h3>
      ...
      <div className="p-8 border-l border-border bg-card/50">
        ...
        <div className="space-y-6 overflow-y-auto max-h-[450px] no-scrollbar pr-2">
          {loading ? (
            ...
            items.map((item, idx) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-secondary/50 border border-border flex items-center justify-center flex-shrink-0 group-hover:border-primary/30 transition-colors">
                  <Zap className={cn("w-5 h-5", idx === 0 ? "text-primary animate-pulse" : "text-muted-foreground opacity-50")} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <p className="text-xs font-bold truncate group-hover:text-primary transition-colors">{item.merchantName}</p>
        ...
        <button className="w-full mt-12 py-3 bg-secondary border border-border rounded-xl text-[10px] font-bold uppercase tracking-widest text-muted-foreground hover:bg-secondary/80 hover:text-foreground transition-all">
